%feature path
featpath=fullfile(basefeatpath,'features_audio_eGeMAPS_turns');
...
parfor p=1:Np,
    Nf=length(labels{p});
    featall{p}=NaN*ones(Nf*2e4,Natt_eGeMAPS);
    labelsall{p}=NaN*ones(1,Nf*2e4);
    install{p}=NaN*ones(1,Nf*2e4);
    cpt=1;
    for f=1:Nf,
        %load features
        data=read_arff(fullfile(featpath,[parinstname{p}{f} '.arff']),0);
        Nins=size(data,1);
        %concatenate them
        featall{p}(cpt:cpt+Nins-1,:)=data(:,2:end);
...
%pre-processing
meanval=mean(featall{1});
stdval=std(featall{1});
featallsp=cell(1,Np);
for p=1:Np,
...
%save prob in files
fid=fopen(fullfile(probas_path,'prob_audio_eGeMAPS_dev'),'wt');
p=2;
Ns=install{p}(end);
for k=1:Ns,
    fprintf(fid,'%g %g %g\n',prob_dev(:,k)');
...
fclose(fid);
% fid=fopen(fullfile(probas_path,'prob_audio_eGeMAPS_test'),'wt');
% p=3;
% Ns=install{p}(end);
% for k=1:Ns,
%     fprintf(fid,'%g %g %g\n',prob_test(:,k)');
