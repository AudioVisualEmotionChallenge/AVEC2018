...
def normFeaturesFile(wSize, wStep, norm, pb):
	valKey = {}
	dFile = v.audioDesc+"Conc/"+v.fconf+"_train_"+str(wSize)+"_"+str(wStep)+".arff"
	if (os.path.isfile(dFile) == True):
		...
		for s in ('test','dev','train'):
			dFile = v.audioDesc+"Conc/"+v.fconf+"_"+s+"_"+str(wSize)+"_"+str(wStep)+".arff"
			if (os.path.isfile(dFile) == True):
				d = arff.load(open(dFile,"rb"))
				for ind, att in enumerate(d['attributes']):
					if (str(att[0]) != "name" and str(att[0]) != "class"):
...
def normFeatures(wSize, wStep):
	...
	pb = 0
	valAtt = {}
	fTests = v.audioDesc+"Conc/"+v.fconf+"_test_"+str(wSize)+"_"+str(wStep)+"_norm.arff"
	fTrains = v.audioDesc+"Conc/"+v.fconf+"_train_"+str(wSize)+"_"+str(wStep)+"_norm.arff"
	fDevs = v.audioDesc+"Conc/"+v.fconf+"_dev_"+str(wSize)+"_"+str(wStep)+"_norm.arff"
