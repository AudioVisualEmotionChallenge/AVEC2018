...
def normFeaturesFile(wSize, wStep, norm, pb):
	valKey = {}
	dFile = v.audioDescConc+v.fconf+"_train_"+str(wSize)+"_"+str(wStep)+".arff"
	if (os.path.isfile(dFile) == True):
		...
		for s in ('test','dev','train'):
			dFile = v.audioDescConc+v.fconf+"_"+s+"_"+str(wSize)+"_"+str(wStep)+".arff"
			if (os.path.isfile(dFile) == True):
				d = arff.load(open(dFile,"rb"))
				for ind, att in enumerate(d['attributes']):
					if (str(att[0]) != "name" and str(att[0]) != "class"):
				...
				f = open(v.audioDescNorm+v.fconf+"_"+s+"_"+str(wSize)+"_"+str(wStep)+"_norm.arff", "w")
...
def normFeatures(wSize, wStep):
	...
	pb = 0
	valAtt = {}
	fTests = v.audioDescConc+v.fconf+"_test_"+str(wSize)+"_"+str(wStep)+"_norm.arff"
	fTrains = v.audioDescConc+v.fconf+"_train_"+str(wSize)+"_"+str(wStep)+"_norm.arff"
	fDevs = v.audioDescConc+v.fconf+"_dev_"+str(wSize)+"_"+str(wStep)+"_norm.arff"
