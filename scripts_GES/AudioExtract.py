...
def extractAudio(wSize, wStep): 
	...
	for f in files :
		...
		if (os.path.isfile(v.aarffd+name+".arff") == False):
			if (os.path.isfile(v.daud+f) == True) :
				output = subprocess.Popen([v.oSmile, "-C", v.aconf, "-I", v.daud+f, "-O", v.aarffd+name+".arff", "2>> ", "log.txt"],stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=current)
				procs.append(output)
				extract += 1
				time.sleep(1)
			else:
